...
(function(root, factory) {
    ...
    function getMockForRequest(handler, requestSettings) {
        ...
        if ($.isFunction(handler.url.test)) {
            ...
            if (!!namespace) {
                var namespacedUrl = [namespace.replace(/(\/+)$/, ''), handler.url.replace(/^(\/+)/, '')].join('/');
    ...
    function processJsonpRequest(requestSettings, mockHandler, origSettings) {
        ...
        if (mockHandler.response && $.isFunction(mockHandler.response)) {
            ...
            $.globalEval('(' +
                ((typeof mockHandler.responseText === 'string') ? ('"' + mockHandler.responseText + '"') : mockHandler.responseText) +
                ')');
        }
        completeJsonpCall(requestSettings, mockHandler, callbackContext, newMock);
        return newMock;
    ...
    function handleAjax(url, origSettings) {
        ...
        for (var k = 0; k < mockHandlers.length; k++) {
            ...
            logger.info(mockHandler, ['MOCK ' + requestSettings.type.toUpperCase() + ': ' + requestSettings.url, $.ajaxSetup({}, requestSettings)]);
    ...
    function clearByUrl(url) {
        var i, len, handler, results = [],
            match = url instanceof RegExp ? function(testUrl) {
        ...
        for (i = 0, len = mockHandlers.length; i < len; i++) {
            ...
            if (!match(handler.url)) {
                ...
                logger.log(handler, ['Clearing mock: ' + (handler && handler.url), handler]);
            }
        }
        return results;
    }
    ...
    $.mockjax.clear = function(i) {
        if (typeof i === 'string' || i instanceof RegExp) {
            ...
            logger.log(mockHandlers[i], ['Clearing mock: ' + (mockHandlers[i] && mockHandlers[i].url), mockHandlers[i]]);
            mockHandlers[i] = null;
        } else {
